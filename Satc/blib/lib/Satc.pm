...
sub describe_msg
{
    ...
    {
	my ($flag_value, $flag_description);
	defined ($flag_value = $msg->{FLAG()}) || ($flag_value = 0);
	$flag_description = "0x" . sprintf ("%04x", $flag_value);
	if (! ($flag_value & 0x0100))
	{
	    $flag_description .= " (blind)";
	    if ($flag_value & 0x0001)
	    {
		$flag_description .= " (method=dial)";
	    }
	    else
	    {
		$flag_description .= " (method=transfer)";
	    };
	}
	else
	{
	    $flag_description .= " (attended)";
	};
