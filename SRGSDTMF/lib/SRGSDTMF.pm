...
sub check_keyseq_match
{
    ...
    my ($front_index, $back_index, $len, $regex, $match, $key);
...
sub _parse_leaf
{
    ...
    $key =~ s/\s+//gs;
    $tag =~ s/\s+//gs;
    length ($key) || return (0, "Empty <item>");
...
sub parse_srgsdtmf_grammar
{
    ...
    if ($grammar =~ /^builtin:dtmf\/(boolean|digits)(\?.*)/)
    {
	##  It's a builtin
	$builtin_type = $1;
	$builtin_args = $2;
	...
	if (defined ($builtin_args))
	{
		...
		{
		    return (0, "Cannot parse builting arg spec \"" .
			    $builtin_args . "\"");
		};
	    };
	};
	...
	if ($builtin_type eq "boolean")
	{
	    ...
	    if (defined ($builtin_args->{"y"}) &&
		($builtin_args->{"y"} =~ /^[0-9\*#]$/so))
	    {
		$keyseq->{"keyset"}[0]{"tags"}{$builtin_args->{"y"}}
		  = "true";
	    };
	    if (defined ($builtin_args->{"n"}) &&
		($builtin_args->{"n"} =~ /^[0-9\*#]$/so))
	    {
	...
	else  ##  builtin type is digits
	{
	    $min = 0;
	    $max = 256;
	    if ((defined ($builtin_args->{"length"})) &&
		($builtin_args->{"length"} =~ /^\d+$/so))
	    {
		$min = $builtin_args->{"length"};
		$max = $builtin_args->{"length"};
	    };
	    if ((defined ($builtin_args->{"minlength"})) &&
		($builtin_args->{"minlength"} =~ /^\d+$/so))
	    {
		$min = $builtin_args->{"minlength"};
	    };
	    if ((defined ($builtin_args->{"maxlength"})) &&
		($builtin_args->{"maxlength"} =~ /^\d+$/so))
	    {
		$max = $builtin_args->{"maxlength"};
	    };
	    if ($max == 0)
	    {
