			       ...
			       const VXIMap  *properties)
{
  ...
  if (type == VXI_MIME_SSML) {
    ...
    //  Extract cookies
    //  impl->inet is the VXIinetInterface*
    //  impl->inet->GetCookieJar (struct VXIinetInterface *pThis, VXIVector **jar, VXIbool *changed)
    VXIVector *cookiejar = NULL;
    if (impl->inet->GetCookieJar (impl->inet, &cookiejar, NULL) !=
	VXIinet_RESULT_SUCCESS)
    {
	if (voiceglue_loglevel() >= LOG_ERR)
	{
	    std::ostringstream logstring;
	    logstring << "Failed getting cookiejar";
	    voiceglue_log ((char) LOG_ERR, logstring);
	};
    };

    VXIpromptResult vg_result =
	voiceglue_prompt (raw_text, properties, cookiejar);

    //  Clean up cookiejar VXIVector at end of call
