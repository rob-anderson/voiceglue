...
/*!
**  Converts a cookie jar to linearized string.
**  Format is "<name1> <value1> <name2> <value2> ... <nameN> <valueN>"
**  where each <nameI> and <valueI> is SATC-escaped.
*/
std::string Cookiejar_to_Std_String (VXIVector *jar)
{
    unsigned i;
    std::ostringstream result;
    for (i = 0; i < VXIVectorLength(jar); ++i)
    {
	VXIMap *cookie_map = (VXIMap *) VXIVectorGetElement (jar, i);
	VXIString *cookie_name =
	    (VXIString *) VXIMapGetProperty (cookie_map, INET_COOKIE_NAME);
	VXIString *cookie_value =
	    (VXIString *) VXIMapGetProperty (cookie_map, INET_COOKIE_VALUE);
	if (i != 0)
	{
	    result << " ";
	};
	result << voiceglue_escape_SATC_string
	    (VXIchar_to_Std_String(VXIStringCStr(cookie_name)).c_str());
	result << " ";
	result << voiceglue_escape_SATC_string
	    (VXIchar_to_Std_String(VXIStringCStr(cookie_value)).c_str());
    };
...
**  @param cookiejar The inet cookiejar
				  ...
				  VXIVector *cookiejar)
{
  ...
  else
  {
      ...
      std::string cookies_utf8 = Cookiejar_to_Std_String (cookiejar);
      if (voiceglue_loglevel() >= LOG_DEBUG)
      {
	  ...
	  << bargein_param_utf8
	  << " "
	  << voiceglue_escape_SATC_string (cookies_utf8)
	  << "\n";
      voiceglue_sendipcmsg (ipc_msg);
  };
